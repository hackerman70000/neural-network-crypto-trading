...
# data setup
df = pd.read_csv('data_day_btc.csv')
...
df.drop(['tradecount','unix','symbol','Volume USDT','Volume BTC'], inplace=True, axis=1)
...
df['uli_50'] = ta.volatility.UlcerIndex(close=df["close"], window=50).ulcer_index()
...
df['exp']  = np.where(  (df['close'] * 1.051 <= df['high'].shift(-1))
                      | (df['close'] * 1.051 <= df['high'].shift(-2))
                      | (df['close'] * 1.051 <= df['high'].shift(-3))
                      | (df['close'] * 1.051 <= df['high'].shift(-4))
                      | (df['close'] * 1.051 <= df['high'].shift(-5))
                      | (df['close'] * 1.051 <= df['high'].shift(-6))
                       ,1 ,0)
print(df['exp'].value_counts())

# deleting unwanted data
#df.drop(df.tail(6).index,inplace=True) #- comment if making predicitons
df.drop(['high','open','close','low'], inplace=True, axis=1)
df.dropna(inplace=True)

# creating prediction dataset
df_train = df.iloc[:df.shape[0]- 1,:]  # declare how many last time frames you wanna predict
df_pred =  df.iloc[df.shape[0]- 1:,:]
print(df_pred)
